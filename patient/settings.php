...
<body>
    ...
    // creates connection and link connection.php 

    $sqlmain= "select * from patient where pemail=?";
    // This line defines an SQL query string ($sqlmain)
    $stmt = $database->prepare($sqlmain);
    // This prepares the SQL query for execution. The prepare() method is part of the mysqli API,
    //  and it prepares the query for execution by the database server, which can then accept parameters in place of the ?
    $stmt->bind_param("s",$useremail);
    //  binds the variable $useremail to the ? placeholder in the SQL query

    $stmt->execute();
    $result = $stmt->get_result();
    // After executing the query, get_result() is used to fetch the result set from the executed query

    $userfetch=$result->fetch_assoc();
    // fetch_assoc() fetches the next row of the result set as an associative array

    $userid= $userfetch["pid"];
    $username=$userfetch["pname"];

    // $userid = $userfetch["pid"];: Gets the patient's ID (pid column) from the database.

     //$username = $userfetch["pname"];: Gets the patient's name (pname column) from the database.
                    ...
                    <td>
                        <p style="font-size: 23px;padding-left:12px;font-weight: 600;">Settings</p>
                            ...
                            <td width="15%">
                                ...
                                // This line executes a simple SQL query to select all rows (*) from the patient table.

                                $doctorrow = $database->query("select  * from  doctor;");
                                // This query retrieves all rows (*) from the doctor table.
                                //  It follows the same structure as the previous query but targets the doctor table instead.

                                $appointmentrow = $database->query("select  * from  appointment where appodate>='$today';");
                                // This query retrieves all rows from the appointment table where the appodate (appointment date) 
                                // is greater than or equal to the current date ($today).

                                $schedulerow = $database->query("select  * from  schedule where scheduledate='$today';");
                                // This query retrieves all rows from the schedule table where the scheduledate matches today's date ($today).
